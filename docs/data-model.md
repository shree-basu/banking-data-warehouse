...
| `fact_account_daily_snapshot` | account and business date | exact `NUMERIC(18,2)` balance |

```mermaid
erDiagram
  DIM_CUSTOMER ||--o{ DIM_ACCOUNT : customer_key
  ...
  DIM_DATE ||--o{ FACT_ACCOUNT_DAILY_SNAPSHOT : date_key
```

Every history row carries natural key, `effective_from`, `effective_to`, `is_current`, `hash_diff`, `batch_id`, `source_file`, and `loaded_at`. Facts resolve the dimension version whose interval contains the event timestamp. Key `0` is an explicit unknown member. A later repair procedure updates facts only when an effective dimension version becomes available.
